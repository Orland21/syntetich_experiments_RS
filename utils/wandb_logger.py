...
def wandb_log_epoch(**kwargs):
    ...
    acc = kwargs['acc']
    c_acc = kwargs['cacc']
    #c_acc_scambio = kwargs['cacc_scambio']
    wandb.log({'acc': acc, "c-acc":c_acc, 'epoch':epoch}) #, 'c-acc-scambio':c_acc_scambio
    ...
    tot_loss = kwargs['totloss']
    wandb.log({'tot_train-loss':tot_loss})
...
def encoding_tuple(x):
 # x is a matrix of shape (n,3) where n is the number of samples
    digit1=x[:,0]
    digit2=x[:,1]
    digit3=x[:,2]
    return digit1*25 + 5 * digit2 +  digit3
