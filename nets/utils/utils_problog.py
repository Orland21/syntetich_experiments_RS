...
def build_worlds_queries_matrix(sequence_len=0, n_digits=0, task='addition'):
    ...
    if task == 'addition':
        ...
        n_queries = len(range(0, 13))#numero di possibili risultati
        ...
        w_q = torch.zeros(n_worlds, n_queries)  # (125, 13)
        for w in range(n_worlds):
            digit1, digit2, digit3 = look_up[w] #ricavo le 3 digit 
            for q in range(n_queries):
                if digit1 + digit2 + digit3 == q:
                    w_q[w, q] = 1
        return w_q
    
    elif task == 'inequality':
        ...
        look_up = {i: c for i, c in zip(range(n_worlds), possible_worlds)} #avendo il numero delle possibili coppie, dizionario che restituisce la coppia caratteristica
        w_q = torch.zeros(n_worlds, n_queries)  # (125, 3)
        for w in range(n_worlds):
            digit1, digit2, digit3 = look_up[w] #ricavo le 2 digit
            result=int(digit1<=2*digit2-digit3)
            add=int(digit1==2*digit2-digit3)
            result=result+add
            for q in range(n_queries):
                if result == q:
                    w_q[w, q] = 1
        ...
        possible_worlds = list(product(range(n_digits), repeat=sequence_len)) #lista a due a due dei possibili addendi, product è il prodotto cartesiano, reapeat è la lunghezza delle combinazioni
        n_worlds = len(possible_worlds) #numero di coppie possibili
        n_queries = len(range(0, 17))#numero di possibili risultati
        look_up = {i: c for i, c in zip(range(n_worlds), possible_worlds)} #avendo il numero delle possibili coppie, dizionario che restituisce la coppia caratteristica
        w_q = torch.zeros(n_worlds, n_queries)  # (125, 3)
        for w in range(n_worlds):
            digit1, digit2, digit3 = look_up[w] #ricavo le 3 digit
            result=digit1+2*digit2-digit3+4
            for q in range(n_queries):
                if result == q:
                    w_q[w, q] = 1
        return w_q
    ...
    elif task == 'modulo_asym':
        possible_worlds = list(product(range(n_digits), repeat=sequence_len)) #lista a due a due dei possibili addendi, product è il prodotto cartesiano, reapeat è la lunghezza delle combinazioni
        n_worlds = len(possible_worlds) #numero di coppie possibili
        n_queries = len(range(0, 4))#numero di possibili risultati
        look_up = {i: c for i, c in zip(range(n_worlds), possible_worlds)} #avendo il numero delle possibili coppie, dizionario che restituisce la coppia caratteristica
        w_q = torch.zeros(n_worlds, n_queries)  # (125, 4)
        for w in range(n_worlds):
            digit1, digit2, digit3 = look_up[w] #ricavo le 2 digit
            result = (digit1 + 2*digit2 - digit3)%4
            for q in range(n_queries):
